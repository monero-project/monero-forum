		...
		if (!$validator->fails()) {
			...
			if (Input::get('wot_register') || GPG::find(Input::get('username'))) {
				if (!Input::get('otcp'))
				{
					...
					$otcp = str_random(40); //generate a random password of 40 chars.
				...
				else if (Input::get('otcp'))
					{
						...
						$hash = $key->password;
						if (Hash::check(Input::get('otcp'), $hash))
						{
							...
							return View::make('user.auth', array('input' => Input::all(), 'message' => $key->message, 'errors' => array('Wrong decrypted message.')));
					}
